...
class YouTubeAuthService {
  ...
  Map<String, String> getAuthHeaders() {
    try {
      if (!Hive.isBoxOpen('youtube_auth')) return {};
      final cookies = Hive.box('youtube_auth').get('cookies');
      if (cookies == null || cookies is! Map) return {};

      final typedCookies = Map<String, String>.from(cookies);

      final cookieString =
          typedCookies.entries.map((e) => '${e.key}=${e.value}').join('; ');

      final sapisid =
          typedCookies['SAPISID'] ?? typedCookies['__Secure-3PAPISID'] ?? '';
  ...
  void saveCookies(Map<String, String> cookies) {
    try {
      ...
      final hasRequired = cookies.containsKey('SAPISID') ||
  ...
  void restoreSession() {
    try {
      ...
      if (cookies != null && cookies is Map) {
        final typedCookies = Map<String, String>.from(cookies);
        if (typedCookies.containsKey('SAPISID') ||
            typedCookies.containsKey('__Secure-3PAPISID')) {
          isSignedIn.value = true;
          fetchUserProfile();
          return;
        }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (actions != null && actions.isNotEmpty) {
          ...
          final activeAccountHeaderRenderer =
              header?['activeAccountHeaderRenderer'] as Map?;

          if (activeAccountHeaderRenderer != null) {
            final accountName =
                activeAccountHeaderRenderer['accountName'] as Map?;
            ...
            final accountPhoto =
                activeAccountHeaderRenderer['accountPhoto'] as Map?;
